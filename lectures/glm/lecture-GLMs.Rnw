...
{How do we convert -3 to a probability? \pause Use the
  inverse-link: \\}
<<inv-logit,size='scriptsize'>>=
p <- exp(logit.p)/(1+exp(logit.p))
p
@
\pause
{Same as:}
<<plogis,size='scriptsize'>>=
plogis(logit.p)
@
\pause
{To go back, use the link function itself:}
<<logit,size='scriptsize'>>=
log(p/(1-p))
qlogis(p)
@
\end{frame}
...
plot(function(x) 5 + -0.08*x, from=0, to=100,
     main="Without the logit transformation",
...
plot(function(x) plogis(5 + -0.08*x), from=0, to=100,
     main="With the logit transformation",
...
plot(function(x) 5 + -0.08*x, from=0, to=100,
     main="Without the log transformation",
...
plot(function(x) exp(5 + -0.08*x), from=0, to=100,
     main="With the log transformation",
