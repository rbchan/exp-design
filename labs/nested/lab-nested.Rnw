...
source("../rnw2pdf.R")
if(1==2) {
  rnw2pdf("lab-nested", clean=FALSE)
...
%        \item We measure Pb levels in multiple blood samples per individual
...
{\bf Treatment effects \par}
$H_0: \alpha_1 = \cdots = \alpha_a = 0$ \\
$H_a:$ at least one inequality \par
\pause
\vspace{0.5cm}
{\bf Random variation among experimental units \par}
$H_0: \sigma^2_B = 0$ \\
$H_a: \sigma^2_B > 0$
...
%\tiny
%<<>>=
%chickData <- read.csv("chickWeights.csv")
%@
\small
{\bf Import data}
<<import>>=
...
<<aov-correct,size='scriptsize'>>=
aov.correct <- aov(larvae ~ Treatment + Error(Plot),
                   data=gypsyData)
@
\pause
<<aov-correct-out,size='scriptsize'>>=
summary(aov.correct)
@
\end{frame}
...
%  {\bf Re-do the ANOVA on the brood means}
%\pause
%\footnotesize
...
\section{Using {\tt lme}}% and {\tt glht}}
