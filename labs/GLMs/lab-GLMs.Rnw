...
plot(p ~ elevation, newdat, type="l", ylim=c(0,1),
...
nd <- data.frame(habitat=c("Oak", "Maple", "Pine"),
                 elevation=250)
pred <- predict(fm1, newdata=nd, se.fit=TRUE, type="link")
...
\pause
%\scriptsize
<<pred-link2,size='footnotesize'>>=
pred.link <- predict(fm2, newdata=newdat, se.fit=TRUE, type="link")
newdat$lambda <- exp(pred.link$fit) # exp is the inverse-link function
...
plot(lambda ~ elevation, newdat, type="l", ylim=c(0,60),
