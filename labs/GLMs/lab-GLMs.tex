...
\hlstd{newdat}\hlopt{$}\hlstd{p} \hlkwb{<-} \hlkwd{plogis}\hlstd{(pred.link}\hlopt{$}\hlstd{fit)}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hlstd{(p} \hlopt{~} \hlstd{elevation, newdat,} \hlkwc{type}\hlstd{=}\hlstr{"l"}\hlstd{,} \hlkwc{ylim}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{1}\hlstd{),}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{nd} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlkwc{habitat}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"Oak"}\hlstd{,} \hlstr{"Maple"}\hlstd{,} \hlstr{"Pine"}\hlstd{),}
                 \hlkwc{elevation}\hlstd{=}\hlnum{250}\hlstd{)}
\hlstd{pred} \hlkwb{<-} \hlkwd{predict}\hlstd{(fm1,} \hlkwc{newdata}\hlstd{=nd,} \hlkwc{se.fit}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{type}\hlstd{=}\hlstr{"link"}\hlstd{)}
\hlstd{bp} \hlkwb{<-} \hlkwd{barplot}\hlstd{(}\hlkwd{plogis}\hlstd{(pred}\hlopt{$}\hlstd{fit),} \hlkwc{ylab}\hlstd{=}\hlstr{"Probability of occurrence"}\hlstd{,}
...
\end{knitrout}
\pause
{\bf To get confidence intervals on (0,1) scale, predict on log
  (link) scale and then backtransform using inverse-link}
\pause
%\scriptsize
\begin{knitrout}\footnotesize
...
\hlstd{newdat}\hlopt{$}\hlstd{lambda} \hlkwb{<-} \hlkwd{exp}\hlstd{(pred.link}\hlopt{$}\hlstd{fit)} \hlcom{# exp is the inverse-link function}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{rpois}\hlstd{(}\hlkwc{n}\hlstd{=}\hlnum{10}\hlstd{,} \hlnum{0.5}\hlstd{)}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{rpois}\hlstd{(}\hlkwc{n}\hlstd{=}\hlnum{10}\hlstd{,} \hlnum{5}\hlstd{)}
\end{alltt}
\begin{verbatim}
##  [1] 6 4 7 7 2 2 3 5 2 2
\end{verbatim}
\end{kframe}
\end{knitrout}
\vfill
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{rpois}\hlstd{(}\hlkwc{n}\hlstd{=}\hlnum{10}\hlstd{,} \hlnum{15}\hlstd{)}
\end{alltt}
\begin{verbatim}
##  [1] 17 18 15 19 13 13 10 20 14 14
\end{verbatim}
\end{kframe}
\end{knitrout}
\end{frame}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{x} \hlkwb{<-} \hlkwd{rpois}\hlstd{(}\hlkwc{n}\hlstd{=}\hlnum{1000}\hlstd{,} \hlnum{3}\hlstd{)}
\hlkwd{hist}\hlstd{(x,} \hlkwc{col}\hlstd{=}\hlstr{"purple"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\centering
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{X} \hlkwb{<-} \hlkwd{model.matrix}\hlstd{(}\hlopt{~}\hlstd{habitat} \hlopt{+} \hlstd{elevation, frogData)}
\end{alltt}
\end{kframe}
\end{knitrout}
\pause
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{beta} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlnum{0.5}\hlstd{,} \hlnum{1}\hlstd{,} \hlopt{-}\hlnum{3}\hlstd{,} \hlnum{0.01}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
\pause
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{sampleSize} \hlkwb{<-} \hlkwd{nrow}\hlstd{(X)}
\hlstd{y} \hlkwb{<-} \hlkwd{rpois}\hlstd{(sampleSize, lambda)}
\end{alltt}
\end{kframe}
\end{knitrout}
\end{frame}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fm3} \hlkwb{<-} \hlkwd{glm}\hlstd{(y} \hlopt{~} \hlstd{habitat} \hlopt{+} \hlstd{elevation,} \hlkwc{family}\hlstd{=poisson, frogData)}
\end{alltt}
\end{kframe}
\end{knitrout}
\scriptsize
...
## 
## Call:
## glm(formula = y ~ habitat + elevation, family = poisson, data = frogData)
## 
## Deviance Residuals: 
##     Min       1Q   Median       3Q      Max  
## -2.5340  -0.6240  -0.3600   0.3649   1.7354  
## 
## Coefficients:
##                Estimate Std. Error z value Pr(>|z|)    
## (Intercept)   0.5869308  0.1091432   5.378 7.55e-08 ***
## habitatMaple  0.9784189  0.0870290  11.242  < 2e-16 ***
## habitatPine  -3.2401513  0.2284357 -14.184  < 2e-16 ***
## elevation     0.0098221  0.0002757  35.629  < 2e-16 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## (Dispersion parameter for poisson family taken to be 1)
## 
##     Null deviance: 7114.008  on 29  degrees of freedom
## Residual deviance:   24.796  on 26  degrees of freedom
## AIC: 147.31
## 
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{CI} \hlkwb{<-} \hlkwd{confint}\hlstd{(fm3)}
\end{alltt}


{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Waiting for profiling to be done...}}\begin{alltt}
\hlkwd{cbind}\hlstd{(CI, beta)}
\end{alltt}
\begin{verbatim}
##                    2.5 %      97.5 %  beta
## (Intercept)   0.36911435  0.79704960  0.50
## habitatMaple  0.80980524  1.15113417  1.00
## habitatPine  -3.71522283 -2.81545807 -3.00
## elevation     0.00929169  0.01037261  0.01
\end{verbatim}
\end{kframe}
\end{knitrout}
\end{frame}
