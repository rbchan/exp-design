...
%  \maketitle
  \LARGE
  \centering \par
  \textcolor{RoyalBlue}{\bf Lab 3 -- Completely Randomized ANOVA \\
...
\begin{center}
  \includegraphics[width=0.85\textwidth]{figure/boxplot1-1}
\end{center}
\end{frame}
...
\definecolor{shadecolor}{rgb}{0.878, 0.918, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{ybar.i} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlkwd{mean}\hlstd{(kick.angle.brandA),} \hlkwd{mean}\hlstd{(kick.angle.brandB),}
            \hlkwd{mean}\hlstd{(kick.angle.brandC),} \hlkwd{mean}\hlstd{(kick.angle.brandD))}
\hlstd{ybar.i}
\end{alltt}
\begin{verbatim}
## [1] 33 43 49 31
...
\hlstd{SSw} \hlkwb{<-} \hlkwd{sum}\hlstd{((y.ij} \hlopt{-} \hlkwd{rep}\hlstd{(ybar.i,} \hlkwc{each}\hlstd{=n))}\hlopt{^}\hlnum{2}\hlstd{)}
\hlstd{SSw}
\end{alltt}
\begin{verbatim}
## [1] 1620
...
\begin{center}
  \includegraphics[width=0.85\textwidth]{figure/barplot2-1}
\end{center}
\end{frame}
...
\begin{center}
  \includegraphics[width=0.7\textwidth]{figure/tukey-1}
\end{center}
\end{frame}
...
%  {\centering \large Do Assignment 1 (it's on ELC). \par}
  ...
  large plot. Heights in meters recorded after several years. The data
  are in the file {\tt treeHt.csv}: \\
  \vfill
  Create an \R~script to do the following:
  \begin{enumerate}[\bf (1)]
  \item Create an ANOVA table using the ``aov'' and ``summary''
    functions. %In other words, do the same thing as before, but the
%    easy way.
  \item Create an ANOVA table (degrees of freedom,
    sums-of-squares, mean-squared error, and F-value) without using
    ``aov''. Compute either the critical value of F or
    the $p$-value.
