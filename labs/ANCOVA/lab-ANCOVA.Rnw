...
dat <- data.frame(diet=g, girth=x)
X <- model.matrix(~diet+girth, dat)
...
\pause
\vspace{0.5cm}
{\bf \large Reorder the levels of the factor, just for convenience}
<<reorder>>=
...
\begin{frame}[fragile]
  ...
  {%\bf %\footnotesize
    Regression lines and CIs can be created using the
    \inr{predict} function. \\}
  \begin{enumerate}[\bf (1)]
    \item Create a new {\tt data.frame} containing a sequence of
        values of the predictor variable {\it girth}
    \item Predict {\it weight} using these values of {\it girth}
...
girth <- dietData$girth
nd <- data.frame(girth=seq(min(girth), max(girth), length=50))
...
plot(weight ~ girth, data=dietData)                   # raw data
lines(fit ~ girth, data=predictionData, lwd=2)        # fitted line
lines(lwr ~ girth, data=predictionData, lwd=2, lty=3) # lower CI
lines(upr ~ girth, data=predictionData, lwd=2, lty=3) # upper CI
@
  \vspace{-9mm}
  \begin{center}
    \includegraphics[width=0.7\textwidth]{figure/plot-fm1-1}
  \end{center}
\end{frame}
...
%   (Control) when girth=0. The other estimates indicate the
...
\begin{frame}[fragile]
  \frametitle{An alternative summary}
  \scriptsize
  {\bf If you want estimates that correspond to the additive model,
    you need to change the {\tt contrasts} argument \\}
\tiny
<<contr2-2>>=
options(contrasts=c("contr.sum", "contr.poly"))
@
\pause
<<summary-lm>>=
summary(lm(weight ~ diet, dietData))
@
\pause
\vfill
\scriptsize
{\centering \bf {\tt (Intercept)} is now the grand mean ($\mu$). The
  other estimates are the $\alpha_i$'s from the additive model. \\}
\end{frame}
\end{comment}
...
%  \small
  \footnotesize
<<center-girth>>=
dietData$girthCentered <- dietData$girth - mean(dietData$girth)
@
\end{frame}
...
\scriptsize %\tiny
<<fm3,size='scriptsize'>>=
fm3 <- lm(weight ~ girthCentered + diet, dietData)
@
{
\pause
\vfill
<<summary-fm3,size='small'>>=
summary(fm3)
@
}
\end{frame}
...
%  \scriptsize
  {%\bf
    Note that the null hypothesis of no diet
...
\begin{frame}[fragile]
  ...
  {%\bf
    Create predictions of {\tt weight} over a sequences of {\tt
      girths}, for every level of {\tt diet}}
  \small
<<predict-girth,size='small'>>=
girthC <- dietData$girthCentered
nd3 <- data.frame(
    diet=rep(c("Control", "Low", "Med", "High"), each=20),
    girthCentered=rep(seq(min(girthC), max(girthC),
                          length=20),
                      times=4))
...
colrs <- rainbow(4) #c("black", "blue", "tan2", "cyan3")
plot(weight ~ girthCentered, dietData, pch=16, cex=1.5,
     col=rep(colrs, each=15))
@
<<scatplot1,fig.show='hide',size='footnotesize'>>=
plot(weight ~ girthCentered, dietData, pch=16, cex=1.5,
     col=rep(colrs, each=15))
lines(fit ~ girthCentered, predData3, subset=diet=="Control",
      col=colrs[1], lwd=2)
lines(fit ~ girthCentered, predData3, subset=diet=="Low", lty=1,
      col=colrs[2], lwd=2)
lines(fit ~ girthCentered, predData3, subset=diet=="Med", lty=1,
      col=colrs[3], lwd=2)
lines(fit ~ girthCentered, predData3, subset=diet=="High", lty=1,
      col=colrs[4], lwd=2)
legend(4, 23, c("High", "Med", "Low", "Control"),
